...
# MODEL DECLARATION
# ================================================
m = pyo.ConcreteModel('3 Component GDP Distillation Model')
...
t = {1: 'A/BC', 2: 'AB/C', 3: 'A/B', 4: 'B/C'}

m.TASKS = pyo.Set(initialize=['A/BC', 'AB/C', 'A/B', 'B/C'],
                  doc='TASKS {t|t is a separation task}')

# mixtures / states
m.STATES = pyo.Set(initialize=['ABC', 'AB', 'BC', 'A', 'B', 'C'])

m.Feed = pyo.Set(initialize=['ABC'])

# TS_s = {tasks t that the state s is able to produce}
m.TS_s = pyo.Set(m.STATES, initialize={'ABC': ('A/BC', 'AB/C'), 'AB': ('A/B',), 'BC': ('B/C',)})

# ST_s = {tasks t that are able to produce state s}
m.ST_s = pyo.Set(m.STATES, initialize={'AB': ('AB/C',), 'BC': ('A/BC',), 'A': ('A/BC', 'A/B'), 'B': ('A/B', 'B/C'), 'C': ('AB/C', 'B/C')})

# FS_F {Columns whose feed is the initial mixture}
m.FS_F = pyo.Set(initialize=['A/BC', 'AB/C'],
...
m.PRE_i = pyo.Set(m.STATES, initialize={'A': ('A/BC', 'A/B'), 'B': ('B/C',)},
...
m.PST_i = pyo.Set(m.STATES, initialize={'B': ('A/B',), 'C': ('AB/C', 'B/C')},
...
m.RECT_s = pyo.Set(m.STATES, initialize={'AB': ('AB/C',), 'A': ('A/BC', 'A/B'), 'B': ('B/C',)},
...
m.STRIP_s = pyo.Set(m.STATES, initialize={'BC': ('A/BC',), 'B': ('A/B',), 'C': ('AB/C', 'B/C')},
...
m.ISTRIP_m = pyo.Set(m.STATES, initialize={'BC': ('A/BC',)},
...
m.rud = pyo.Var(
    m.TASKS, m.r, doc='possible active Underwood root (r) in task t',
    within=pyo.NonNegativeReals,
    bounds=(0, 500)
)
...
m.rec_comp = pyo.Param(m.COMP, initialize={'A': rec, 'B': rec, 'C': rec},
...
@m.LogicalConstraint(m.COMP,
                     doc = """Logic 7 and Logic 8: If a final product is produced by exactly one contribution,
                     the heat exchanger associated with this product must be selected.""")
def logic7(m, i):
    if i in m.PRE_i:
...
@m.LogicalConstraint(m.COMP,
                     doc="""If a given final state is produced by 2 tasks, then there is
                     no heat exchnager associated with that state""")
def logic9(m, i):
    if i in m.PRE_i:
...
@m.Constraint(m.ISTATE,
              doc ="""Intermediate heat exchanger logic: cannot have a heat exchanger for an
              intermediate state if the state is not produced by a task""")
def logic10(m, s):
